...
function clean_or_create_files() {
	...
	rm $1.bin $1.iso $1.img > /dev/null 2>&1
}

function compile() {
	# Compila o arquivo e gera o binário
	nasm -f bin -o $1.bin src/$1.asm > /dev/null 2>&1
	...
	dd if=$1.bin of=$1.img seek=0 count=1 conv=notrunc
	# Move a imagem para dentro da pasta auxiliar e gera uma iso bootável 
	mv $1.img iso_temp/
	genisoimage -quiet -V 'Skirtshot OS' -input-charset iso8859-1 -o $1.iso -b $1.img -hide $1.img iso_temp/
}

function run() {
	# Executa a iso no emulador qemu
	qemu-system-x86_64 -cdrom $1.iso
}
